...
from core.loss import sequence_loss, polar_loss
...
class PLWrap(pl.LightningModule):
    def __init__(self, cfg):
        ...
        if cfg.last_stage_ckpt is not None:
            ...
            # ckpt_dict["pytorch-lightning_version"] = "2.1.4"
    ...
    #计算对极距离版训练
    # def training_step(self, batch, batch_idx):
    #     image1, image2, mesh, cr, pose1, pose2 = [x for x in batch]
    #     image1 = image1.cuda()
    #     image2 = image2.cuda()
    #     mesh = mesh.cuda()
    #     cr = cr.cuda()
    ...
    #         image2 = (image2 + stdv * torch.randn(*image2.shape).cuda()).clamp(0.0, 255.0)
    #     flow_predictions = self.model(image1, image2)
    #     # print(f'flow_len: {len(flow_predictions)}')
    #     # print(f'flow_shape: {flow_predictions[0].shape}')
    #     loss = polar_loss(flow_predictions, mesh, cr, pose1, pose2, self.cfg)
    #     self.log("loss", loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
    #     return loss
...
def train(cfg):
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=save_dir+'/ckpt', every_n_train_steps=5, save_last=True)

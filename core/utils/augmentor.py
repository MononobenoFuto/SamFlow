...
class SparseMeshAugmentor:
    ...
    def spatial_transform(self, img1, img2):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        UpCorrection(correction, [scale_x, scale_y, 0.0, 0.0])
        ...
        if self.do_flip:
            if np.random.rand() < 0.5: # h-flip
                img1 = img1[:, ::-1]
                img2 = img2[:, ::-1]
                UpCorrection(correction, [-1.0, 1.0, img1.shape[1], 0.0])


        #crop的影响 x'=x-x0 y'=y-y0
        margin_y = 20
        margin_x = 50
        ...
        img1 = img1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2 = img2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        UpCorrection(correction, [1.0, 1.0, -x0, -y0])
        return img1, img2, correction


    def __call__(self, img1, img2):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, correction = self.spatial_transform(img1, img2)
...
def UpCorrection(correction, c):
    correction[0] *= c[0]
    correction[1] *= c[1]
    correction[2] = correction[2] * c[0] + c[2]
    correction[3] = correction[3] * c[1] + c[3]
    return correction